...
const calculateTotal = (income, expenses) => {income > expenses ? (totalBalance.textContent = `$${(income + expenses).toFixed(2)}`)
...
const render = (obj) => {
  let Html;
  obj.amount > 0? (Html = `<li class="plus"><span>${obj.text}</span> +${obj.amount}</li>`)
    : (Html = `<li class="minus"><span>${obj.text}</span> -${Math.abs(obj.amount)}</li>`);
...
renderOnload = (allValues) => {
  ...
  if (allValues.length > 0) {
    allValues.forEach((obj) => {
      ...
      parsedObj.amount > 0 ? (Html = `<li class="plus"><span>${parsedObj.text}</span>+${parsedObj.amount.toFixed(2)}</li>`)
        : (Html = `<li class="minus"><span>${parsedObj.text}</span>-${Math.abs(parsedObj.amount)}</li>`);
      list.insertAdjacentHTML("beforeEnd", Html);
    });
  }
};
